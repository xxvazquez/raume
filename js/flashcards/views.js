...
window.RaumeStudy.flashcards.views = (function () {
  ...
  function renderHelp(panel) {
    panel.innerHTML =
      ...
      '<li>Studying a word the built-in tables don\'t have? Add it under <span class="fc-legend-term">Your vocabulary</span> on the Customize page (the sliders icon) — one at a time or a paste / CSV import — then it\'s a normal word you can add here. Signed in, you can build whole tables of your own; deleting a word or table you made there is a real delete (there\'s no review history on the word itself to keep).</li>' +
      '</ul></div>' +
